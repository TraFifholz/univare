// Import Modules
import { UnivareActor } from "./actor/actor.js";
...
Hooks.once('init', async function() {
  ...
  CONFIG.Combat.initiative = {
    formula: "1d20 + @others.init.value",
    decimals: 2
  };
...
Hooks.once("ready", async function() {
  ...
  Hooks.on("hotbarDrop", (bar, data, slot) => createUnivareMacro(data, slot));
});
 ...
 * @returns {Promise}
 */
async function createUnivareMacro(data, slot) {
  if (data.type !== "Item") return;
  if (!("data" in data)) return ui.notifications.warn("You can only create macro buttons for owned Items");
  const item = data.data;

  // Create the macro command
  const command = `game.univare.rollItemMacro("${item.name}");`;
